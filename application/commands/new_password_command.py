...
class NewPasswordCommand(BaseCommand):
    def __init__(self, username, domain, password_generator, password_store, extra_id=None, password=None):
    ...
    def execute(self):
        ...
        try:
            if self.password:
                password_entry = self.password_service.add_existing_password(self.username, self.domain, self.password, self.extra_id)
                print(f"Added existing password: {password_entry}")
            else:
                password_entry = self.password_service.create_password(self.username, self.domain, self.extra_id)
